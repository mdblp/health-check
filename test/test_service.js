...
describe('Global health check service', () => {
    ...
    describe('When all services are responding correctly', () => {
        let statusResults = [];
        it('the service should return a success code and an array of 4 items, including "yourloops" item', (done) => {
            chai.request(svc.server).get('/status').end((err,res) => {
                res.should.have.status(Service.responseCode.success);
        ...
        it('all 3 services (excluding "yourloops") should report a status success', () => {
            // we start at 1 as "yourloops" is #0
            statusResults[1].status.should.eql(Service.responseStatus.success);
            statusResults[2].status.should.eql(Service.responseStatus.success);
            statusResults[3].status.should.eql(Service.responseStatus.success);
        });
        it('all 3 services (excluding "yourloops") should report a "details" = "OK"', () => {
        ...
        it('"yourloops" item should report a status success', () => {
            statusResults[0].status.should.eql(Service.responseStatus.success);
        });
    ...
    describe('When one service is down', () => {
        ...
        it('the service should return a status faillure and an array of 4 items, including "yourloops" item', (done) => {
            chai.request(svc.server).get('/status').end((err,res) => {
                res.should.have.status(Service.responseCode.failure);
                statusResults = res.body;
                res.body.should.be.a('array');
                res.body.length.should.eql(4);
                done();
        ...
        it('should return an error for shoreline/auth', () => {           
            ...
            authStatus[0].status.should.eql(Service.responseStatus.failure);
            authStatus[0].error.should.eql("500 - \"service is down\"");
        });
        it('"yourloops" item should report a status code failure', () => {
            statusResults[0].status.should.eql(Service.responseStatus.failure);
        });
        it('"yourloops" item should have version = ' + ylpVersion, () => {
            statusResults[0].version.should.eql(ylpVersion);
        });
    ...
    describe('When one service does not respond in a timely manner (timeout)', () => {
        ...
        it('the service should return a code failure and an array of 4 items, including "yourloops" item', (done) => {
            chai.request(svc.server).get('/status').end((err,res) => {
                res.should.have.status(Service.responseCode.failure);
                statusResults = res.body;
                res.body.should.be.a('array');
